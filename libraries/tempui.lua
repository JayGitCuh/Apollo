...
local dwLighting = game:GetService("Lighting")
...
function library:Watermark(text)
    ...
    bar.BackgroundColor3 = Color3.fromRGB(69, 23, 255)
...
function library:AddConnection(a,b)
    local s = a:Connect(b)
    return s
end
...
function library:notify(text)
    ...
    playing = true
    spawn(function()
        Notif:Notify(text, 3, "notification") -- notification, alert, error, success, information
        playing = false
    end)
end
    ...
    function tab:createGroup(pos,groupname) -- newTab[pos == 0 and "left" or "right"] 
        ...
        function group:addSlider(args,sub)
            ...
            local function updateValue(value)
                value = math.clamp(utility.round(value, float), args.min, args.max)
                if library.colorpicking then return end
				if value ~= 0 then
			...
			local function updateScroll()
                ...
                while inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and menu.Enabled do runService.RenderStepped:Wait()
					...
					updateValue(math.clamp(utility.round(value, float), args.min, args.max))
                end
...
local mss = Drawing.new('Circle')
mss.Radius = 2
mss.ZIndex = 9999999
mss.Color = Color3.fromRGB(69, 23, 255)
mss.Filled = true
library:AddConnection(game:GetService("RunService").RenderStepped, function()
    mss.Visible = menu.bg.Visible
    mss.Position = Vector2_new(dwMouse.X, dwMouse.Y + 36)
end)
...
function library:createConfig()
    ...
    writefile('apollo/configs/'..name..'.cfg',game:GetService("HttpService"):JSONEncode(jig))
...
function library:saveConfig()
    ...
    if name:sub(#name - 3, #name) == '.cfg' then
        writefile('apollo/configs/'..name,game:GetService("HttpService"):JSONEncode(jig))
...
function library:loadConfig()
    local name = library.flags["selected_config"]
    if not isfile('apollo/configs/'..name) then
        library:notify("Config file not found!")
        return
    end
    local config = game:GetService("HttpService"):JSONDecode(readfile('apollo/configs/'..name))
...
function library:refreshConfigs()
    local tbl = {}
    for i,v in next, listfiles("apollo/configs/") do
        table.insert(tbl,string.sub(v, 17))
    end
    library.options["selected_config"].refresh(tbl)
end

function library:deleteConfig()
    if isfile('apollo/configs/'..library.flags["selected_config"]) then
        delfile('apollo/configs/'..library.flags["selected_config"])
        library:refreshConfigs()
    end
end
return library, menu
